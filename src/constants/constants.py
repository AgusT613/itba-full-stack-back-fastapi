import os

# SQLITE
SQLITE_FILE_NAME = "src/database.db"
SQLITE_URL = f"sqlite:///{SQLITE_FILE_NAME}"

# TABLE NAMES
BRANCH_OFFICES = "branch_offices"
USERS = "users"
BANK_ACCOUNTS = "bank_accounts"

# .ENV
ALGORITHM = os.getenv("ALGORITHM")
SECRET_KEY = os.getenv("AUTH_SECRET_KEY")
ACCESS_TOKEN_EXPIRE_MINUTES = 30

# AUTH FASTAPI
TOKEN_URL = "/api/auth/token"

# ENDPOINTS
API_PREFIX = "/api"

AUTH_ROUTER_TAG = "auth"
AUTH_PREFIX = "/auth"
AUTH_GET_CURRENT_USER = "/users/me"
AUTH_GET_CURRENT_ACTIVE_USER = "/users/me/active"
AUTH_POST_TOKEN = "/token"
AUTH_POST_REGISTER = "/register"

ITBANK_ROUTER_TAG = "itbank"
ITBANK_PREFIX = "/itbank"
ITBANK_BRANCH_OFFICES = "/branch-offices"

HOME_BANKING_PREFIX = "/homebanking"
ACCOUNTS_PREFIX = "/accounts"
TRANSFERS_PREFIX = "/transfers"

# COMPLETE ENDPOINT
ITBANK_BRANCH_OFFICES_COMPLETE_ENDPOINT = (
    API_PREFIX + ITBANK_PREFIX + ITBANK_BRANCH_OFFICES
)
ITBANK_HOMEBANKING_COMPLETE_ENDPOINT = API_PREFIX + HOME_BANKING_PREFIX
ITBANK_ACCOUNTS_COMPLETE_ENDPOINT = API_PREFIX + HOME_BANKING_PREFIX + ACCOUNTS_PREFIX
ITBANK_TRANSFERS_COMPLETE_ENDPOINT = API_PREFIX + HOME_BANKING_PREFIX + TRANSFERS_PREFIX
AUTH_REGISTER_COMPLETE_ENDPOINT = API_PREFIX + AUTH_PREFIX + AUTH_POST_REGISTER
AUTH_GET_TOKEN_COMPLETE_ENDPOINT = API_PREFIX + AUTH_PREFIX + AUTH_POST_TOKEN

# EXCEPTION MESSAGES
INVALIDE_CREDENTIALS = "Could not validate credentials"
INACTIVE_USER = "Inactive user"
INCORRECT_USERNAME_OR_PASSWORD = "Incorrect username or password"
ALREADY_REGISTERED_USER = "Username already registered"
